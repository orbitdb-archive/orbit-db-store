...
class Replicator extends EventEmitter {
  ...
  async _processOne (entry) {
    ...
    const exclude = []
    const log = await Log.fromEntryHash(this._store._ipfs, this._store.identity, this._store.identities, hash, { logId: this._store._oplog.id, access: this._store.access, length: batchSize, exclude })
